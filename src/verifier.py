...
class Verifier():
    ...
    def get_valid_proof(self, txid: str, nodes: list, k: int):
        ...
        # keep querying nodes until the entropy tx has k confirmations and we get a valid Blink proof
        valid_proof = False
        while not valid_proof:         
            for node in nodes:
                # get entropy tx info
                tx_info = node.get_raw_transaction(txid) 
                # wait until the entropy tx appears in the ledger and can thus be queried
                if tx_info is None:
                    continue
    ...
    def check_proof(self, txid: str,  tx_info: str, node: Node, k: int):
        ...
        check_received_data(entropy_block_header, schema_block_header, "Block header")
